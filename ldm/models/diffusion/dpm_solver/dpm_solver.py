...
import math
from ldm.modules.attention import get_global_heat_map, clear_heat_maps, get_rank,edit_rank,clear_rank,clear_lamb,edit_lamb,next_heat_map,get_lamb
...
def clear_mask_global():
    global mask_global
    mask_global =None


mid_latent_code=defaultdict(list)
def get_mid_latent_code():
    global mid_latent_code
    return mid_latent_code

def edit_mid_latent_code(r):
    global mid_latent_code
    mid_latent_code=r

def clear_mid_latent_code():
    global mid_latent_code
    mid_latent_code = defaultdict(list)
...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, cond=None,scond=None, pmask=None):
        ...
        if cond is None:
            ...
            if scond !=None:
                output = model(x, t_input, cond,scond, pmask, **model_kwargs)
            else:
            ...
            if guidance_scale == 1. or unconditional_condition is None:
                ...
                if src_condition!=None:
                    ...
                    if isinstance(condition, (list, dict)):
                        c_in = [unconditional_condition, condition, src_condition]
                    else:
                        c_in = torch.cat([unconditional_condition, condition, src_condition])
                    noise_uncond, noise, noise_edit = noise_pred_fn(x_in, t_in, cond=c_in).chunk(3)
                    lamb = get_lamb()# lambda
                    clamp_rate=1
                    subed = (noise - noise_edit).abs_().mean(dim=[0, 1])
                    ...
                    noise_mid = mask * (lamb * noise + float(1 - lamb) * noise_edit) + (1 - mask) * noise_edit

                    return noise_uncond + guidance_scale * (noise_mid - noise_uncond)#ouo

                else:
                    if scond!=None:# prompt-to-prompt
                        x_in = torch.cat([x] * 2)
                        t_in = torch.cat([t_continuous] * 2)

                        c_in = torch.cat([unconditional_condition, condition])
                        sc_in = torch.cat([unconditional_condition, scond])

                        noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in,scond=sc_in, pmask=pmask).chunk(2)
